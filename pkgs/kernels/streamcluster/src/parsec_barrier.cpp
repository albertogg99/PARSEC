...
#define NOT_IMPLEMENTED() Not_Implemented(__FUNCTION__,__FILE__,__LINE__);//NOLINT

static inline void Not_Implemented(const char* function,const char* file,unsigned int line) {
  fmt::print("ERROR: Something in function {} in file {}, line {} is not implemented.\n", function, file, line);
  exit(1);
}
...
int parsec_barrier_init(parsec_barrier_t *barrier, const parsec_barrierattr_t *attr, unsigned count) {
  ...
  static_assert(PARSEC_BARRIER_SERIAL_THREAD != 0); 
  ...
  if(attr!=nullptr && *attr==PARSEC_PROCESS_PRIVATE){
    NOT_IMPLEMENTED();//NOLINT
  } 
  ...
  int rv = pthread_mutex_init(&barrier->mutex, nullptr);
  if(rv != 0){
    return rv;
  } 
  rv= pthread_cond_init(&barrier->cond, nullptr);
  return rv;
}

int parsec_barrier_destroy(parsec_barrier_t *barrier) {
  assert(barrier!=NULL); //NOLINT

  int rv = pthread_mutex_destroy(&barrier->mutex);
  if(rv != 0){
    return rv;
  } 
...
int parsec_barrierattr_destroy(const parsec_barrierattr_t *attr) {
  if(attr==nullptr){
    return EINVAL;
  } 
  //simply do nothing
...
int parsec_barrierattr_init(const parsec_barrierattr_t *attr) {
  if(attr==nullptr){
    return EINVAL;
  } 
  //simply do nothing
...
int parsec_barrier_wait(parsec_barrier_t *barrier) {
  ...
  int rv = pthread_mutex_lock(&barrier->mutex);
  if(rv != 0){
    return rv;
  }
  ...
  if(barrier->is_arrival_phase == 0) {
    pthread_mutex_unlock(&barrier->mutex);
    volatile spin_counter_t i=0;
    while(barrier->is_arrival_phase == 0 && i<SPIN_COUNTER_MAX){
      i++;
    } 
    ...
    while(barrier->is_arrival_phase == 0) {
      rv = pthread_cond_wait(&barrier->cond, &barrier->mutex);
      if(rv != 0) {
        pthread_mutex_unlock(&barrier->mutex);
        return rv;
  ...
  int master = static_cast<int>(barrier->n == 0); //Make first thread at barrier the master
  barrier->n++;
  ...
  return (master != 0 ? PARSEC_BARRIER_SERIAL_THREAD : 0);
}
