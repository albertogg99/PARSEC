...
void outcenterIDs( Points* centers, long* centerIDs, char* outfile ) {
  ...
  fclose(fp);
  free(is_a_median); // Added to solve a memory leak
}
...
// Added to solve memory leakm, from here

  for(int i = 0; i < QUEUE_SIZE; i ++){  
    Points* points = &(queue.points[i]);
    /*float* block;
    for( int j = 0; j < chunksize; j++ ) {
      block = points->p[j].coord;
    }*/
#ifdef TBB_VERSION
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
  ...
  delete[](outfilename); // Added to solve memory leak
  delete[](infilename);  // Added to solve memory leak
  delete stream;
